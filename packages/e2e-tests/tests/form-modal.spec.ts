...
test.describe('Form Modal Stories', () => {
  ...
  test('should navigate to Form Modal default story', async ({ formModalPage }) => {
    await formModalPage.navigateToDefaultStory();
    await formModalPage.expectStoryTitleToContain('Демонстрация модалки с формой');
  });

  test('should test form submission with valid data', async ({ formModalPage }) => {
    await formModalPage.navigateToDefaultStory();
  ...
  test('should test form with special characters', async ({ formModalPage }) => {
    await formModalPage.navigateToDefaultStory();
    
    await formModalPage.testFormSubmission('José María', 'jose.maria@example.com');
  });
